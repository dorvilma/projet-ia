...
export class DataEngineerHandler extends AgentHandler {
  readonly role = 'DATA_ENGINEER' as const;
  readonly name = 'Data Engineer';
  ...
  protected async execute(ctx: AgentContext, task: TaskMessage): Promise<AgentResult> {
    ...
    try {
      ...
      const result = {
        analysis: `Analyzed data engineering task: ${task.title}`,
